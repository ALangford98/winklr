...
- [x] Gift Suggestions — guests submit a name/quantity/email; owner approves (with an editable final quantity) or rejects from the Items panel or the exported site's owner view; on the exported site the owner can also fix the name, replace/add the photo, and edit already-approved suggestions; approved suggestions persist (derived from the suggestion records) and render in a "Suggested Gifts" section pinned at the bottom
