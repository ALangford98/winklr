...
- [x] Gift Suggestions — guests submit a name/quantity/email; owner approves (with an editable final quantity) or rejects from the Items panel or the exported site's owner view; on the exported site the owner can also fix the name, replace/add the photo, and edit already-approved suggestions; approved suggestions persist (derived from the suggestion records) and render in a "Suggested Gifts" section pinned at the bottom; a suggester can opt to reserve their suggested gift themselves (unchecked by default) and the reservation is created under their name on approval
...
- [x] Cash Fund — guests can pledge a cash contribution instead of (or alongside) items; owner controls the heading/message, whether the running total and goal progress are shown, and optional payment/bank details text (up to two labelled accounts, e.g. a local and an international one); guests can record a pledge so the owner (and other guests, if enabled) can see what's been contributed. No real payment processing — money changes hands outside the app
